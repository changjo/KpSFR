...
def run(args):
    ...
    new_size = (
        int(ORIGINAL_WIDTH / args.crop_ratio),
        int(ORIGINAL_HEIGHT / args.crop_ratio),
    )
    ...
    print(T_2)
    T_2[0, 2] = -homo_points[0, 0]
    T_2[1, 2] = -homo_points[0, 1]
    print(T_2)
    # H = T_2 @ H
    ...
    out_dir = Path(args.homography).parent.parent.joinpath("bev").as_posix()
    # out_dir = osp.join(osp.dirname(osp.dirname(args.image)), "bev")
    make_dir(out_dir)
    out_img_filename = osp.join(out_dir, osp.splitext(osp.basename(args.image))[0] + "_bev.jpg")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--homography", type=str, required=True, help="Path to homography matrix")
    parser.add_argument("--image", type=str, required=True, help="Path to image")
    parser.add_argument("--width", type=int, required=True, help="Width of birds eye view")
    parser.add_argument("--height", type=int, required=True, help="Height of birds eye view")
    parser.add_argument("--out_scale", type=int, default=20, help="Scale of output image")
    parser.add_argument(
        "--crop",
        type=int,
        nargs="+",
    ...
    parser.add_argument("--crop_ratio", default=1.0, type=float, help="Crop ratio")
